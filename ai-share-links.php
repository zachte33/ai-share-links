...
define('AI_SHARE_LINKS_VERSION', '1.1.2');
...
final class AI_Share_Links {
    ...
    private function init_hooks() {
        ...
        // Remove AJAX handlers since we're not using them anymore
        // add_action('wp_ajax_ai_share_toggle_page', array($this, 'ajax_toggle_page'));
        
        // Hook to display share buttons on pages
    ...
    public function activate() {
        ...
        $default_options = array(
            ...
            'page_slugs' => '' // New option for page slugs
        );
    ...
    public function enqueue_frontend_assets() {
        ...
        // Add inline CSS directly to wp_head
        add_action('wp_head', function() {
            echo '<style id="ai-share-links-css">' . $this->get_frontend_css() . '</style>';
        }, 100);
        
        // Add inline JS directly to wp_footer  
    ...
    /**
     * Add share buttons to content (posts only)
     *
    ...
    /**
     * Add share buttons to pages (if enabled)
     *
     * @since 1.1.0
     */
    public function add_page_buttons() {
        if (!is_page() || is_admin() || $this->is_builder_preview()) {
            return;
        }
        ...
        // If no slugs are specified, don't show on any pages
        if (empty($page_slugs)) {
            return;
        }
        ...
        // Convert slugs string to array and clean it up
        $allowed_slugs = array_map('trim', explode(',', $page_slugs));
        $allowed_slugs = array_filter($allowed_slugs); // Remove empty values
        
        // Check if current page slug is in the allowed list
        ...
        // Add buttons to bottom of page content
        echo '<script type="text/javascript">';
        echo 'document.addEventListener("DOMContentLoaded", function() {';
        echo 'var content = document.querySelector(".entry-content, .page-content, main");';
        echo 'if (content) {';
        echo 'var buttonsDiv = document.createElement("div");';
        echo 'buttonsDiv.innerHTML = ' . json_encode($buttons) . ';';
        echo 'content.appendChild(buttonsDiv.firstChild);';
        echo '}';
        echo '});';
        echo '</script>';
    }
    ...
    public function sanitize_options($input) {
        ...
        $sanitized['scheme'] = in_array($input['scheme'], array('blue', 'salmon', 'forest', 'seafoam', 'cosmic', 'brand', 'brand-transparent', 'solid-navy', 'midnight-aurora'), true) ? $input['scheme'] : 'blue';
        ...
        $sanitized['page_slugs'] = sanitize_text_field($input['page_slugs']);
        ...
        <div class="wrap">
            ...
            <form method="post" action="options.php">
                ...
                <table class="form-table" role="presentation">
                    ...
                    <tr>
                        <th scope="row">
                            <label for="ai_share_position"><?php esc_html_e('Button Position (Posts Only)', AI_SHARE_LINKS_TEXT_DOMAIN); ?></label>
                        </th>
                        <td>
                            ...
                            <p class="description"><?php esc_html_e('This setting applies to all single blog posts automatically.', AI_SHARE_LINKS_TEXT_DOMAIN); ?></p>
                        </td>
                    </tr>
                    
                    <tr>
                        <th scope="row">
                            <label for="ai_share_page_slugs"><?php esc_html_e('Page Slugs', AI_SHARE_LINKS_TEXT_DOMAIN); ?></label>
                        </th>
                        <td>
                            <textarea id="ai_share_page_slugs" name="ai_share_links_options[page_slugs]" rows="3" class="large-text" placeholder="about-us, contact, services/web-design"><?php echo esc_textarea($options['page_slugs']); ?></textarea>
                            <p class="description"><?php esc_html_e('Enter page slugs or paths separated by commas. Examples: about-us, contact, services/web-design, parent-page/child-page', AI_SHARE_LINKS_TEXT_DOMAIN); ?></p>
                        </td>
                    </tr>
    ...
    private function get_color_schemes() {
        return apply_filters('ai_share_links_color_schemes', array(
            ...
            'brand' => array(
                'name' => __('Navy Light', AI_SHARE_LINKS_TEXT_DOMAIN),
                'description' => __('Deep navy to bright blue - bold and professional brand colors', AI_SHARE_LINKS_TEXT_DOMAIN)
            ),
            'brand-transparent' => array(
                'name' => __('Transparent', AI_SHARE_LINKS_TEXT_DOMAIN),
                'description' => __('Minimal transparent style - adapts to any background', AI_SHARE_LINKS_TEXT_DOMAIN)
            ),
            'solid-navy' => array(
                'name' => __('Solid Navy', AI_SHARE_LINKS_TEXT_DOMAIN),
                'description' => __('Solid deep navy blue - clean and authoritative', AI_SHARE_LINKS_TEXT_DOMAIN)
            ),
            'midnight-aurora' => array(
                'name' => __('Midnight Aurora', AI_SHARE_LINKS_TEXT_DOMAIN),
                'description' => __('Deep navy to electric cyan - striking and futuristic', AI_SHARE_LINKS_TEXT_DOMAIN)
            )
        ));
    }
